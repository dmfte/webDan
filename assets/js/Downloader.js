export class Downloader {
    ...
    constructor(buttonEl, canvas, captureCallback = null) {
        this.btn = buttonEl;
        this.canvas_out = canvas;
        this.#captureCallback = captureCallback;

        this.btn.addEventListener('click', async (evt) => {
            const a = document.createElement('a');
            a.download = this.#get_file_name();
            const scaled_canvas = await this.#getScaledCanvas(this.canvas_out, this.#scale);
            a.href = this.#captureCallback
                ? this.#captureCallback(this.canvas_out)
                : this.canvas_out.toDataURL('image/png');
            a.click();
        });
    }

    #getScaledCanvas(canvas, scale) {
        return new Promise ((res, rej) => {
            if (scale == 1) return res(canvas);
        });
    }
    scale(scale = 1) {
